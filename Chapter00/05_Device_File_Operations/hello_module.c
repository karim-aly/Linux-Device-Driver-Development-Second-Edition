...
static char kernel_buffer[KERNEL_BUFFER_SIZE] = 
        "This is a message from the kernel inside the the kernel buffer!\n";
...
static ssize_t hello_driver_write(struct file *file, const char __user *user_buffer,
                                  size_t len, loff_t *ppos) {
    pr_info("%s: Write is called!, i_ino=%lu\n", __func__,  file->f_inode->i_ino);
    return len; /* Not doing anything with the data */
}

static ssize_t hello_driver_read(struct file *file, char __user *user_buffer,
                                 size_t count, loff_t *ppos) {
    unsigned long ret_val;
    pr_info("%s: Read is called!, count=%lu\n", __func__, count);

    if (!(file->f_mode & FMODE_READ)) {
        return -EBADF;
    }
    if (*ppos > KERNEL_BUFFER_SIZE || count == 0) {
        return 0; // return -EFBIG also works
    }

    if ((*ppos + count) > KERNEL_BUFFER_SIZE) {
        count = KERNEL_BUFFER_SIZE - *ppos;
    }

    ret_val = copy_to_user(user_buffer, kernel_buffer + *ppos, count);
    if (ret_val != 0) {
        return -EFAULT;
    }
    *ppos += count;

    pr_info("%s: return with count=%lu, offset=%lld is called!\n", __func__, count, *ppos);
...
static int setup_cdev(struct cdev *dev, int minor,
                       struct file_operations *fops) {
    int ret_val = -1;

    // MKDEV call creates a device number
    // i.e. combination of major and minor number
    // dev_t devno = MKDEV(MAJOR(device_number), minor);
    dev_t devno = device_number;
...
static int __init hello_init(void) {
    ...
    ret_val = setup_cdev(&hello_cdev, device_number, p_hello_fops);
    if (ret_val < 0) {
